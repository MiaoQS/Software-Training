...
def printing_1():
    ...
    bar = (Bar().add_xaxis([
        ...
        "数量",
        y_number).set_global_opts(title_opts=opts.TitleOpts(title="动车非动车统计图")))

    bar.render('print_1.html')
...
def printing_2():
    ...
    bar = (Bar().add_xaxis([
        ...
        "数量",
        y_number).set_global_opts(title_opts=opts.TitleOpts(title="列车时间分布")))

    bar.render('print_2.html')


# def sql_write():
#     for name_i in all_name:
#         new_station = User(Name=name_i, Numbering=dic_name[name_i])
